...
echo "Setting password for $username. Please follow the prompts:"
passwd $username
...
# Amazon Linux uses 'wheel' for admin privileges by default.
if grep -q -E "^wheel:" /etc/group; then
    usermod -aG wheel $username
    echo "$username added to 'wheel' group."
else
    echo "Failed to add user to sudo group. Group 'wheel' does not exist." 1>&2
    exit 2
fi
